...
namespace Setsuna
{
	class SaveData
	{
		...
		public bool Open(String filename, bool force)
		{
			...
			{
				if(hash[i] != mBuffer[mBuffer.Length - hash.Length + i])
				{
					check = false;
					break;
				}
		...
		public bool Save()
		{
			...
			WriteBytes((uint)(mBuffer.Length - 16), CheckSum());
		...
		{
			byte[] calc = new byte[mBuffer.Length - 192];
			Array.Copy(mBuffer, 192, calc, 0, calc.Length - 16);
			var md5 = new System.Security.Cryptography.MD5CryptoServiceProvider();
			byte[] hash = md5.ComputeHash(calc);
			md5.Clear();
			return hash;
		}
